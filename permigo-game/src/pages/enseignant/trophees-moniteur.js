// ═══════════════════════════════════════════════════════════════
// Enseignant — Trophées professionnels
// 12 jalons pédagogiques : bronze → argent → or → platine → diamant
// Ton Linear/Notion — aucun cosmétique, que des outils utiles débloqués
// ═══════════════════════════════════════════════════════════════
import { sb } from "@/auth/auth.js";
import { getCurUser } from "@/auth/cur-user.js";
import { esc } from "@/utils/escape.js";
import { toast } from "@/components/common/toast.js";
import { track } from "@/services/analytics.js";
import { navigate } from "@/router.js";
import { icon } from "@/utils/icons.js";
import { haptic } from "@/utils/haptic.js";

// ─── Définition des trophées ─────────────────────────────────
const TIERS = {
  bronze: {
    label: "Bronze",
    color: "#cd7f32",
    bg: "rgba(205,127,50,.12)",
    border: "rgba(205,127,50,.35)",
  },
  argent: {
    label: "Argent",
    color: "#9ca3af",
    bg: "rgba(156,163,175,.12)",
    border: "rgba(156,163,175,.35)",
  },
  or: {
    label: "Or",
    color: "#f59e0b",
    bg: "rgba(245,158,11,.12)",
    border: "rgba(245,158,11,.35)",
  },
  platine: {
    label: "Platine",
    color: "#7dd3fc",
    bg: "rgba(125,211,252,.12)",
    border: "rgba(125,211,252,.35)",
  },
  diamant: {
    label: "Diamant",
    color: "#a78bfa",
    bg: "rgba(167,139,250,.14)",
    border: "rgba(167,139,250,.45)",
  },
};

const TROPHEES = [
  // ─ Bronze
  {
    id: "premiere_seance",
    tier: "bronze",
    iconName: "play",
    name: "Premier pas",
    desc: "Première séance enregistrée dans PermiGo.",
    check: (d) => d.totalVals >= 1,
    progress: (d) => ({ v: Math.min(1, d.totalVals), max: 1 }),
  },
  {
    id: "dix_comps",
    tier: "bronze",
    iconName: "check-circle",
    name: "10 compétences",
    desc: "Un début qui compte — 10 acquisitions validées avec tes élèves.",
    check: (d) => d.totalVals >= 10,
    progress: (d) => ({ v: Math.min(10, d.totalVals), max: 10 }),
  },
  {
    id: "premier_eleve",
    tier: "bronze",
    iconName: "user-check",
    name: "Premier élève mobilisé",
    desc: "Ton premier élève actif dans l'app ces 30 derniers jours.",
    check: (d) => d.studentsActive >= 1,
    progress: (d) => ({ v: Math.min(1, d.studentsActive), max: 1 }),
  },
  // ─ Argent
  {
    id: "streak_7",
    tier: "argent",
    iconName: "flame",
    name: "Semaine active",
    desc: "7 jours consécutifs d'activité pédagogique enregistrée.",
    check: (d) => d.streak >= 7,
    progress: (d) => ({ v: Math.min(7, d.streak), max: 7 }),
  },
  {
    id: "cinquante_comps",
    tier: "argent",
    iconName: "trending-up",
    name: "50 compétences",
    desc: "La régularité commence à faire une vraie différence.",
    check: (d) => d.totalVals >= 50,
    progress: (d) => ({ v: Math.min(50, d.totalVals), max: 50 }),
  },
  {
    id: "cinq_eleves",
    tier: "argent",
    iconName: "users",
    name: "Classe en formation",
    desc: "5 élèves suivis simultanément dans PermiGo.",
    check: (d) => d.studentsTotal >= 5,
    progress: (d) => ({ v: Math.min(5, d.studentsTotal), max: 5 }),
  },
  // ─ Or
  {
    id: "cent_comps",
    tier: "or",
    iconName: "award",
    name: "100 compétences",
    desc: "Référent pédagogique — 100 acquisitions accompagnées.",
    check: (d) => d.totalVals >= 100,
    progress: (d) => ({ v: Math.min(100, d.totalVals), max: 100 }),
  },
  {
    id: "streak_30",
    tier: "or",
    iconName: "zap",
    name: "Mois sans faille",
    desc: "30 jours consécutifs actifs sans interruption.",
    check: (d) => d.streak >= 30,
    progress: (d) => ({ v: Math.min(30, d.streak), max: 30 }),
  },
  {
    id: "dix_eleves",
    tier: "or",
    iconName: "users",
    name: "Portefeuille solide",
    desc: "10 élèves accompagnés en parallèle.",
    check: (d) => d.studentsTotal >= 10,
    progress: (d) => ({ v: Math.min(10, d.studentsTotal), max: 10 }),
  },
  // ─ Platine
  {
    id: "deux_cent_comps",
    tier: "platine",
    iconName: "shield",
    name: "200 compétences",
    desc: "Expertise pédagogique avérée — tu formes des conducteurs solides.",
    check: (d) => d.totalVals >= 200,
    progress: (d) => ({ v: Math.min(200, d.totalVals), max: 200 }),
  },
  {
    id: "classe_complete",
    tier: "platine",
    iconName: "check-circle",
    name: "Classe au complet",
    desc: "Tous tes élèves (3 minimum) actifs sur les 30 derniers jours.",
    check: (d) => d.studentsTotal >= 3 && d.studentsActive >= d.studentsTotal,
    progress: (d) => ({
      v: d.studentsActive,
      max: Math.max(3, d.studentsTotal),
    }),
  },
  // ─ Diamant
  {
    id: "expert_remc",
    tier: "diamant",
    iconName: "star",
    name: "Expert REMC certifié",
    desc: "300 validations — le palier ultime. Statut Expert REMC débloqué.",
    check: (d) => d.totalVals >= 300,
    progress: (d) => ({ v: Math.min(300, d.totalVals), max: 300 }),
  },
];

// ─── CSS ─────────────────────────────────────────────────────
const STYLE = `<style>
.tm {
  max-width: 580px;
  margin: 0 auto;
  padding-bottom: 100px;
  background: var(--bg);
  color: var(--ink);
  font-family: 'Inter', sans-serif;
}

/* Header */
.tm-hd {
  padding: 18px 16px 16px;
  background: var(--su);
  border-bottom: 1px solid var(--bo);
  display: flex;
  align-items: center;
  gap: 12px;
}
.tm-back {
  width: 44px; height: 44px;
  border-radius: 10px;
  border: 1.5px solid var(--bo);
  background: var(--su);
  cursor: pointer;
  display: flex; align-items: center; justify-content: center;
  color: var(--ink);
  flex-shrink: 0;
  transition: background .12s;
  -webkit-tap-highlight-color: transparent;
}
.tm-back:active { background: var(--bg2); }
.tm-hd-info { flex: 1; min-width: 0; }
.tm-hd-title {
  font: 800 17px/1.2 'Plus Jakarta Sans', sans-serif;
  color: var(--ink);
  letter-spacing: -.02em;
}
.tm-hd-sub {
  font: 500 12px/1 'Inter', sans-serif;
  color: var(--mu2);
  margin-top: 2px;
}

/* Hero stats */
.tm-hero {
  padding: 20px 16px 16px;
  background: var(--su);
  border-bottom: 1px solid var(--bo);
}
.tm-hero-row {
  display: flex;
  align-items: center;
  gap: 16px;
  margin-bottom: 14px;
}
.tm-hero-ring {
  width: 64px; height: 64px;
  flex-shrink: 0;
  position: relative;
}
.tm-ring-svg { display: block; }
.tm-ring-track { fill: none; stroke: var(--bo); stroke-width: 5; }
.tm-ring-fill  { fill: none; stroke-width: 5; stroke-linecap: round;
  transform: rotate(-90deg); transform-origin: 50% 50%;
  transition: stroke-dasharray .8s cubic-bezier(.2,.7,.3,1) .2s; }
.tm-ring-count {
  position: absolute; inset: 0;
  display: flex; flex-direction: column;
  align-items: center; justify-content: center;
  line-height: 1;
}
.tm-ring-val {
  font: 800 16px/1 'Plus Jakarta Sans', sans-serif;
  color: var(--ink);
}
.tm-ring-max {
  font: 500 9px/1 'Inter', sans-serif;
  color: var(--mu2);
  margin-top: 2px;
}
.tm-hero-txt { flex: 1; }
.tm-hero-label {
  font: 700 15px/1.3 'Plus Jakarta Sans', sans-serif;
  color: var(--ink);
  margin-bottom: 4px;
}
.tm-hero-caption {
  font: 500 12px/1.4 'Inter', sans-serif;
  color: var(--mu2);
}

/* Barre de progression globale */
.tm-global-bar {
  height: 6px;
  background: var(--bo2);
  border-radius: 3px;
  overflow: hidden;
}
.tm-global-fill {
  height: 100%;
  background: linear-gradient(90deg, var(--a), var(--adk));
  border-radius: 3px;
  transition: width .8s cubic-bezier(.2,.7,.3,1) .3s;
}

/* Sections par tier */
.tm-section { padding: 0 16px; margin-top: 20px; }
.tm-tier-hd {
  display: flex;
  align-items: center;
  gap: 8px;
  margin-bottom: 10px;
}
.tm-tier-dot {
  width: 8px; height: 8px;
  border-radius: 50%;
  flex-shrink: 0;
}
.tm-tier-lbl {
  font: 700 11px/1 'Inter', sans-serif;
  text-transform: uppercase;
  letter-spacing: .08em;
  color: var(--mu2);
}
.tm-tier-count {
  margin-left: auto;
  font: 600 11px/1 'Inter', sans-serif;
  color: var(--mu2);
}

/* Cards grid */
.tm-cards {
  display: flex;
  flex-direction: column;
  gap: 8px;
}
.tm-card {
  display: flex;
  align-items: center;
  gap: 14px;
  padding: 14px 16px;
  background: var(--su);
  border: 1.5px solid var(--bo);
  border-radius: 16px;
  box-shadow: 0 1px 2px rgba(10,13,26,.04);
  transition: transform .12s, border-color .15s;
  -webkit-tap-highlight-color: transparent;
  animation: tmCardIn .4s cubic-bezier(.2,.7,.3,1) both;
}
.tm-card:active { transform: scale(.98); }
@keyframes tmCardIn {
  from { opacity: 0; transform: translateY(6px); }
  to   { opacity: 1; transform: translateY(0); }
}
@media (prefers-reduced-motion: reduce) { .tm-card { animation: none; } }

.tm-card.tm-unlocked { border-color: var(--tc-border, var(--bo)); background: var(--tc-bg, var(--su)); }

.tm-card-ico {
  width: 44px; height: 44px;
  border-radius: 12px;
  display: flex; align-items: center; justify-content: center;
  flex-shrink: 0;
  background: var(--tc-bg, var(--bg2));
  color: var(--tc-color, var(--mu2));
  border: 1.5px solid var(--tc-border, var(--bo));
}
.tm-card.tm-locked .tm-card-ico {
  background: var(--bg2);
  color: var(--mu5);
  border-color: var(--bo);
}

.tm-card-body { flex: 1; min-width: 0; }
.tm-card-name {
  font: 700 14px/1.2 'Plus Jakarta Sans', sans-serif;
  color: var(--ink);
  margin-bottom: 3px;
}
.tm-card.tm-locked .tm-card-name { color: var(--mu2); }
.tm-card-desc {
  font: 500 12px/1.4 'Inter', sans-serif;
  color: var(--mu2);
  white-space: nowrap;
  overflow: hidden;
  text-overflow: ellipsis;
}

/* Progress bar sur card */
.tm-card-prog {
  margin-top: 7px;
  display: flex;
  align-items: center;
  gap: 7px;
}
.tm-card-prog-bar {
  flex: 1;
  height: 4px;
  background: var(--bo2);
  border-radius: 2px;
  overflow: hidden;
}
.tm-card-prog-fill {
  height: 100%;
  background: var(--tc-color, var(--a));
  border-radius: 2px;
  transition: width .6s cubic-bezier(.2,.7,.3,1);
}
.tm-card-prog-txt {
  font: 600 10px/1 'IBM Plex Mono', monospace;
  color: var(--mu2);
  white-space: nowrap;
}

/* Badge check (débloqué) */
.tm-card-check {
  width: 28px; height: 28px;
  border-radius: 50%;
  background: var(--tc-color, var(--a));
  display: flex; align-items: center; justify-content: center;
  flex-shrink: 0;
  color: #fff;
}
/* Badge lock (verrouillé et loin) */
.tm-card-lock {
  width: 28px; height: 28px;
  border-radius: 50%;
  background: var(--bg2);
  display: flex; align-items: center; justify-content: center;
  flex-shrink: 0;
  color: var(--mu5);
}
</style>`;

// ─── Mount ────────────────────────────────────────────────────
export async function mount(root) {
  const me = getCurUser();
  if (!me || me.role !== "enseignant") return;

  track("page_view", { page: "trophees_moniteur" });

  root.innerHTML = `${STYLE}
<div class="tm anim-slide-up">
  <div class="tm-hd">
    <button class="tm-back" id="tm-back" aria-label="Retour">${icon("arrow-left", { size: 20, strokeWidth: 2.5 })}</button>
    <div class="tm-hd-info">
      <div class="tm-hd-title" tabindex="-1">Trophées</div>
      <div class="tm-hd-sub">Jalons pédagogiques</div>
    </div>
  </div>
  <div class="tm-skel" style="height:104px;margin:16px;border-radius:16px;background:linear-gradient(90deg,var(--bg3),var(--bg5) 50%,var(--bg3));background-size:200% 100%;animation:shimmer 1.4s ease-in-out infinite"></div>
  <style>@keyframes shimmer{from{background-position:200% 0}to{background-position:-200% 0}}</style>
</div>`;

  root.querySelector("#tm-back")?.addEventListener("click", () => {
    haptic("tap");
    navigate("/parcours");
  });

  try {
    const since30d = new Date(Date.now() - 30 * 86400000).toISOString();

    const [profileRes, countRes, studentsRes, activeRes] = await Promise.all([
      sb
        .from("profiles")
        .select("prenom, streak_pro_days")
        .eq("id", me.id)
        .maybeSingle(),
      sb
        .from("validations")
        .select("id", { count: "exact", head: true })
        .eq("validated_by", me.id),
      sb
        .from("profiles")
        .select("id", { count: "exact", head: true })
        .eq("enseignant_id", me.id)
        .eq("role", "eleve"),
      sb
        .from("validations")
        .select("eleve_id")
        .eq("validated_by", me.id)
        .gte("validated_at", since30d),
    ]);

    const d = {
      totalVals: countRes.count ?? 0,
      streak: profileRes.data?.streak_pro_days ?? 0,
      studentsTotal: studentsRes.count ?? 0,
      studentsActive: new Set(
        (activeRes.data || []).map((v) => v.eleve_id).filter(Boolean),
      ).size,
      prenom: profileRes.data?.prenom ?? "",
    };

    _render(root, d);
  } catch (e) {
    console.error("[trophees-moniteur]", e);
    toast("Erreur de chargement", "error");
  }
}

// ─── Render ──────────────────────────────────────────────────
function _render(root, d) {
  const results = TROPHEES.map((t) => ({
    ...t,
    unlocked: t.check(d),
    prog: t.progress(d),
  }));

  const unlockedCount = results.filter((t) => t.unlocked).length;
  const total = results.length;
  const pct = Math.round((unlockedCount / total) * 100);

  // Circumference for ring: r=25, c=2πr≈157
  const C = 2 * Math.PI * 25;
  const filled = (pct / 100) * C;

  // Group by tier
  const byTier = {};
  for (const tier of Object.keys(TIERS)) byTier[tier] = [];
  results.forEach((t) => byTier[t.tier]?.push(t));

  const tierSections = Object.entries(TIERS)
    .map(([key, cfg]) => {
      const list = byTier[key] || [];
      if (list.length === 0) return "";
      const doneCount = list.filter((t) => t.unlocked).length;
      return `
      <div class="tm-section">
        <div class="tm-tier-hd">
          <span class="tm-tier-dot" style="background:${cfg.color}"></span>
          <span class="tm-tier-lbl">${cfg.label}</span>
          <span class="tm-tier-count">${doneCount}/${list.length}</span>
        </div>
        <div class="tm-cards">
          ${list.map((t, i) => _renderCard(t, cfg, i)).join("")}
        </div>
      </div>`;
    })
    .join("");

  root.innerHTML = `${STYLE}
<div class="tm anim-slide-up">
<div class="tm-hd">
  <button class="tm-back" id="tm-back" aria-label="Retour">${icon("arrow-left", { size: 20, strokeWidth: 2.5 })}</button>
  <div class="tm-hd-info">
    <div class="tm-hd-title" tabindex="-1">Trophées</div>
    <div class="tm-hd-sub">${unlockedCount} débloqué${unlockedCount > 1 ? "s" : ""} sur ${total}</div>
  </div>
</div>

<div class="tm-hero">
  <div class="tm-hero-row">
    <div class="tm-hero-ring">
      <svg class="tm-ring-svg" width="64" height="64" viewBox="0 0 64 64" aria-hidden="true">
        <circle class="tm-ring-track" cx="32" cy="32" r="25"/>
        <circle class="tm-ring-fill" cx="32" cy="32" r="25"
          stroke="var(--a)"
          stroke-dasharray="${filled.toFixed(1)} ${C.toFixed(1)}"/>
      </svg>
      <div class="tm-ring-count">
        <span class="tm-ring-val">${unlockedCount}</span>
        <span class="tm-ring-max">/${total}</span>
      </div>
    </div>
    <div class="tm-hero-txt">
      <div class="tm-hero-label">${_headline(unlockedCount, total, d)}</div>
      <div class="tm-hero-caption">${pct}% des jalons atteints</div>
    </div>
  </div>
  <div class="tm-global-bar">
    <div class="tm-global-fill" style="width:${pct}%"></div>
  </div>
</div>

${tierSections}
</div>`;

  root.querySelector("#tm-back")?.addEventListener("click", () => {
    haptic("tap");
    navigate("/parcours");
  });
}

// ─── Card render ─────────────────────────────────────────────
function _renderCard(t, cfg, i) {
  const delay = `${i * 50}ms`;
  const prog = t.prog;
  const pct = prog.max > 0 ? Math.round((prog.v / prog.max) * 100) : 0;
  const isClose = !t.unlocked && pct >= 30;

  const cssVars =
    t.unlocked || isClose
      ? `style="--tc-color:${cfg.color};--tc-bg:${cfg.bg};--tc-border:${cfg.border};animation-delay:${delay}"`
      : `style="animation-delay:${delay}"`;

  const cls = t.unlocked ? "tm-card tm-unlocked" : "tm-card tm-locked";

  const rightSlot = t.unlocked
    ? `<div class="tm-card-check" aria-label="Débloqué">${icon("check", { size: 13, strokeWidth: 3 })}</div>`
    : `<div class="tm-card-lock" aria-label="Verrouillé">${icon("lock", { size: 12, strokeWidth: 2 })}</div>`;

  const progressBar = !t.unlocked
    ? `<div class="tm-card-prog">
        <div class="tm-card-prog-bar"><div class="tm-card-prog-fill" style="width:${pct}%;background:${isClose ? cfg.color : "var(--bo2)"}"></div></div>
        <span class="tm-card-prog-txt">${prog.v}/${prog.max}</span>
       </div>`
    : "";

  return `
  <div class="${cls}" ${cssVars}>
    <div class="tm-card-ico">${icon(t.iconName, { size: 18, strokeWidth: 2 })}</div>
    <div class="tm-card-body">
      <div class="tm-card-name">${esc(t.name)}</div>
      <div class="tm-card-desc">${esc(t.desc)}</div>
      ${progressBar}
    </div>
    ${rightSlot}
  </div>`;
}

// ─── Headline contextuel ─────────────────────────────────────
function _headline(unlocked, total, d) {
  if (unlocked === 0) return "Lance-toi — enregistre ta première séance";
  if (unlocked === total) return "Tous les trophées débloqués 🏆";
  if (unlocked >= total * 0.7)
    return `Presque au bout — ${total - unlocked} restant${total - unlocked > 1 ? "s" : ""}`;
  if (d.prenom)
    return `${esc(d.prenom)}, ${unlocked} trophée${unlocked > 1 ? "s" : ""} dans ta collection`;
  return `${unlocked} trophée${unlocked > 1 ? "s" : ""} débloqué${unlocked > 1 ? "s" : ""}`;
}
