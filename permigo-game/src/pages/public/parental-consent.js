...
export async function mount(root) {
  ...
  if (!token) return renderError(root, "Ce lien de consentement est incomplet. Demande à ton enfant de te renvoyer le lien.");
  ...
  try {
    ...
    return renderError(root, "Impossible de vérifier ce lien pour le moment. Réessaie dans quelques instants.");
  }

  if (!req) return renderError(root, "Ce lien n'est plus valide : le consentement a peut-être déjà été donné, ou le lien a expiré.");
...
function renderForm(root, req, token) {
  ...
  const inscritLe = req.inscrit_le ? new Date(req.inscrit_le).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' }) : '—';
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      const { error } = await sb.rpc('accept_parental_consent', { p_token: token });
      if (error) throw error;
      ...
      toast(/invalid_or_used/i.test(e?.message || '') ? 'Ce lien a déjà été utilisé.' : 'Erreur — réessaie.', 'error', 4000);
      btn.disabled = false;
