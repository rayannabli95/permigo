...
-- Résout la surcharge ambiguë de public.log_session (deux signatures en prod,
-- cf. 0007_rpc_recovery.sql lignes 2087 et 2151), qui provoquait l'erreur
-- "Could not choose the best candidate function between: public.log_session(...)".
--
-- log_session_v2 = corps VERBATIM de la signature B (6 paramètres, 0007:2151),
-- simplement renommé. Aucune logique métier modifiée (caps horaires, rôles,
-- validations 'a_valider', note_enseignant = p_comment). Les appels nommés du
-- front (p_eleve_id, p_duration_minutes, p_session_date, p_competence_ids,
-- p_comment, p_notes) ciblent désormais une signature UNIQUE.
--
-- Filet de sécurité : on NE drope PAS les anciennes signatures log_session.
-- Drop différé en prod APRÈS confirmation que plus aucun client ne les appelle
...
CREATE OR REPLACE FUNCTION public.log_session_v2(
  ...
  p_competence_ids text[] DEFAULT NULL::text[],
...
-- ─── Drop différé (NE PAS exécuter tant que des clients appellent encore
--     l'ancienne log_session). À lancer en prod après bascule complète : ──────
