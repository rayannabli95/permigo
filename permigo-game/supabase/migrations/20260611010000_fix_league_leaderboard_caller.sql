-- ═══════════════════════════════════════════════════════════════
-- FIX : get_league_leaderboard ne retournait JAMAIS rien.
-- Cause : v_caller := auth.uid() puis lookup `profiles WHERE id = v_caller`,
-- or dans ce projet profiles.id ≠ auth.uid() (la correspondance est
-- profiles.auth_id). v_ecole_id restait NULL → RETURN immédiat, et
-- is_me n'était jamais vrai. Vérifié en prod : 0/38 profils ont id = auth_id.
-- Fix : résoudre le profil appelant via auth_id (une seule ligne change
-- dans le DECLARE/lookup, le corps des requêtes est inchangé).
-- ═══════════════════════════════════════════════════════════════
