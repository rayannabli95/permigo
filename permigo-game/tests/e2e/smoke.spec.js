...
test.describe('Login', () => {
  ...
  test('connexion élève réussie → accueil chargé', async ({ page }) => {
    ...
    await expect(page.locator('.acc-bonjour')).toBeVisible();
    await expect(page.locator('.acc-xp-pill')).toBeVisible();
  });
});
...
test.describe('Parcours REMC', () => {
  ...
  test('fiche se referme avec le bouton ×', async ({ page }) => {
    ...
    await expect(page.locator('#bsheet')).not.toHaveClass(/open/, { timeout: 4_000 });
  });
});

// ─── Push Notifications ──────────────────────────────────────────────
test.describe('Push notifications — gating', () => {
  test('banner push absent au 1er login (sans permigo_has_validated)', async ({ page }) => {
  ...
  test('banner push absent si permission déjà refusée (denied)', async ({ page, context }) => {
  ...
  test('toggle change l\'état opted-out dans localStorage', async ({ page, context }) => {
    // Accorder la permission notifications pour ce test
    await context.grantPermissions(['notifications']);
    await loginAsEleve(page);
    // Forcer l'état initial : push activé, pas d'opt-out
    await page.evaluate(() => {
      localStorage.removeItem('permigo_push_optout');
    });
    await page.evaluate(() => { location.hash = '#/profil'; });
    ...
    const optedOut = await page.evaluate(() => localStorage.getItem('permigo_push_optout'));
