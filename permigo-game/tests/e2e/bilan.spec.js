...
async function goToBilan(page, eleveId) {
  await page.evaluate((id) => {
    location.hash = `#/bilan/${id}`;
  }, eleveId);

  const found = await page
    .waitForSelector(".bl", { timeout: 8_000 })
    .catch(() => null);
  if (!found) {
...
test.describe("Bilan trimestriel enseignant", () => {
  ...
  test("page bilan se charge (ou affiche erreur gracieuse si RPC absente)", async ({
    page,
  }) => {
    const errors = [];
  ...
  test("bilan : 4 KPI affichent des valeurs (pas NaN ni vide)", async ({
    ...
    if (isNoData) {
      test.skip(
        true,
        "RPC get_bilan_data non disponible ou élève sans données",
      );
      return;
    }
