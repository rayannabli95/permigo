...
async function goToInsights(page) {
  ...
  const found = await page.waitForSelector('.ins-page, .ins-widgets', { timeout: 10_000 }).catch(() => null);
  if (!found) {
...
test.describe('Insights enseignant', () => {
  ...
  test('les 4 KPI affichent des valeurs numériques (pas NaN ni vide)', async ({ page }) => {
    await loginAsEnseignant(page);
    await goToInsights(page);
  ...
  test('clic sur un élève en stagnation → navigation vers livret', async ({ page }) => {
    await loginAsEnseignant(page);
    await goToInsights(page);
    ...
    if (!await eleveRow.isVisible({ timeout: 3_000 }).catch(() => false)) {
      test.skip(true, 'Aucun élève en stagnation disponible en données de test');
      return;
    }
  ...
  test('mode rapide — sélection 2 élèves + comp → bouton confirmer activé', async ({ page }) => {
    await loginAsEnseignant(page);
  ...
  test('mode rapide — fermeture avec Escape ou backdrop', async ({ page }) => {
    ...
    await expect(page.locator('.mr-overlay')).toHaveCount(0, { timeout: 2_000 });
  });
