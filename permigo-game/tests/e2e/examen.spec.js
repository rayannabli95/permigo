...
async function loginAsEleve(page) {
  ...
  // (.gt-root « Visite express ») intercepte les clics (#exam-date-save
  // inatteignable → timeout, flake prouvé par les traces).
...
async function goToExamen(page) {
  ...
  const found = await page
    .waitForSelector(".exam", { timeout: 8_000 })
    .catch(() => null);
  if (!found) {
...
test.describe("Page Examen élève", () => {
  ...
  test('countdown : sans date → affiche le CTA "Choisir ma date"', async ({
    page,
  }) => {
    await loginAsEleve(page);
    ...
    await expect(page.locator(".exam-choose-btn")).toBeVisible({
      timeout: 5_000,
    });
    await expect(page.locator(".exam-countdown-tiles")).toHaveCount(0);
  });

  test("countdown : avec date future → affiche les 3 tuiles (jours/heures/min)", async ({
    page,
  }) => {
    await loginAsEleve(page);
  ...
  // 4 critères depuis l'alignement du verdict sur le moniteur :
  // parcours, streak, score quiz, révision (l'ancien 5e critère « heures »
  // a été retiré du produit).
  test("checklist : 4 lignes affichées, aucun NaN", async ({ page }) => {
    await loginAsEleve(page);
    await goToExamen(page);
  ...
  test("saisir et enregistrer une date → countdown se rafraîchit", async ({
    page,
  }) => {
    await loginAsEleve(page);
    await page.evaluate((key) => localStorage.removeItem(key), LS_KEY_DATE);
    ...
    // Clic en DOM direct + délais élargis : sous forte charge (suite complète
    // en parallèle), l'animation d'ouverture dépasse les 3 s d'origine.
    await page.locator(".exam-choose-btn").evaluate((el) => el.click());
    await page.waitForSelector(".exam-date-input-wrap.open", {
      timeout: 8_000,
    });
