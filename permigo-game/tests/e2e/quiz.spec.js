...
test.describe('Quiz — overlay UI', () => {
  ...
  test('quiz overlay s\'ouvre et affiche une question', async ({ page }) => {
    ...
    await page.evaluate(() => { location.hash = '#/quiz/C1a/post_validation'; });

    // Attendre soit la page quiz soit l'overlay quiz-engine
    const quizSelector = '.quiz-overlay, .qp-root, #quiz-root';
    const found = await page.waitForSelector(quizSelector, { timeout: 15_000 }).catch(() => null);
    ...
    if (!found) {
      ...
      console.warn('[quiz spec] Route #/quiz non définie — routing à compléter');
  ...
  test('les options de quiz sont cliquables', async ({ page }) => {
    ...
    const overlayOrPage = await page.waitForSelector(
      '.quiz-overlay, .qp-root, .quiz-opt',
      { timeout: 15_000 }
    ).catch(() => null);
  ...
  test('quiz-overlay — bouton Continuer ferme le modal', async ({ page }) => {
    ...
    const overlay = await page.waitForSelector('.quiz-overlay', { timeout: 15_000 }).catch(() => null);
    if (!overlay) {
    ...
    if (await closeBtn.isVisible({ timeout: 5_000 }).catch(() => false)) {
      ...
      await expect(page.locator('.quiz-overlay')).toHaveCount(0, { timeout: 3_000 });
    }
  });
});
