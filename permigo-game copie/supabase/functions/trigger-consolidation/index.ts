...
Deno.serve(async () => {
  ...
  const { data: existingNotifs } = await supabase
    ...
    .in('user_id', dues.map(v => v.eleve_id));
  ...
  if (!toProcess.length) {
    return new Response(JSON.stringify({ ok: true, processed: 0, skipped: dues.length }));
  }
  ...
  return new Response(JSON.stringify({ ok: true, processed: toProcess.length }));
});
