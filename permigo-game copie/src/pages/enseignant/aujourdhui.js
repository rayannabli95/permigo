...
// ─── Statuts labels ───────────────────────────────────────────────
const STATUT_LABEL = {
  acquis:         { label: 'Acquis',         color: 'var(--gr)',  bg: 'var(--grp)' },
  en_cours:       { label: 'En cours',       color: 'var(--am)',  bg: 'var(--amp)' },
  a_retravailler: { label: 'À retravailler', color: 'var(--rd)',  bg: 'var(--rdp)' },
};
...
const STYLE = `<style>
  ...
  .aj-kpi.kpi-acquis .aj-kpi-val { color: var(--gr); }
  .aj-kpi.kpi-eleves .aj-kpi-val { color: var(--pu); }

  /* Section title */
  .aj-section-title {
    font: 700 13px/1 var(--fn);
  ...
  .aj-act-row:hover { background: var(--ap); }
  ...
  .aj-eleve-row {
    ...
    box-shadow: var(--s0);
    cursor: pointer;
    transition: border-color var(--t), box-shadow var(--t), transform .2s;
    min-height: 44px;
  }
  .aj-eleve-row:hover {
    border-color: var(--a);
  ...
  .aj-cta-btn:hover { box-shadow: 0 6px 22px rgba(99,102,241,.45); }
  .aj-cta-btn:active { transform: scale(.98); }
  ...
  .aj-skel-kpi { height: 90px; background: var(--su); border-radius: var(--rl); animation: aj-pulse 1.4s ease-in-out infinite; }
  .aj-skel-bloc { height: 160px; background: var(--su); border-radius: var(--rl); animation: aj-pulse 1.4s ease-in-out infinite; animation-delay: .1s; }
  @keyframes aj-pulse {
    0%, 100% { opacity: 1; }
    50% { opacity: .5; }
  }
  ...
  return `
    <div class="aj-act-row">
      ...
      <div class="aj-act-info">
        ...
        <div class="aj-act-comp">${esc(val.competence_id || '—')}</div>
      </div>
