...
// ─── CSS ────────────────────────────────────────────────────────
const STYLE = `<style>
  .vp { padding: 20px 16px 120px; max-width: 580px; margin: 0 auto; }
  .vp-hd { margin-bottom: 28px; }
  .vp-h1 { font: 800 26px/1.2 'Plus Jakarta Sans', sans-serif; color: #fff; margin: 0 0 6px; }
  .vp-sub { font: 500 14px/1.4 'Inter', sans-serif; color: #94a3b8; margin: 0; }

  .step { margin-bottom: 32px; }
  .step-lbl { font: 700 11px/1 'IBM Plex Mono', monospace; letter-spacing: .1em; text-transform: uppercase; color: #6366f1; margin-bottom: 6px; }
  .step-ttl { font: 700 18px/1.3 'Plus Jakarta Sans', sans-serif; color: #fff; margin: 0 0 16px; display: flex; align-items: center; flex-wrap: wrap; gap: 8px; }
  .step-progress { font: 600 12px/1 'IBM Plex Mono', monospace; color: #10b981; background: rgba(16,185,129,.12); border: 1px solid rgba(16,185,129,.25); border-radius: 20px; padding: 4px 10px; }

  /* Élèves */
  .eleves-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(110px, 1fr)); gap: 10px; }
  .eleve-card { padding: 16px 10px; background: rgba(99,102,241,.07); border: 1.5px solid rgba(99,102,241,.18); border-radius: 16px; cursor: pointer; text-align: center; transition: all .2s; }
  .eleve-card:active { transform: scale(.96); }
  .eleve-card.selected { background: rgba(99,102,241,.18); border-color: #6366f1; box-shadow: 0 0 0 3px rgba(99,102,241,.18); }
  .eleve-av { width: 48px; height: 48px; border-radius: 50%; background: linear-gradient(135deg, #6366f1, #8b5cf6); margin: 0 auto 10px; display: flex; align-items: center; justify-content: center; color: #fff; font: 700 18px/1 'Plus Jakarta Sans', sans-serif; }
  .eleve-prenom { font: 600 13px/1.2 'Inter', sans-serif; color: #fff; margin-bottom: 4px; }
  .eleve-hrs { font: 500 11px/1 'IBM Plex Mono', monospace; color: #64748b; }

  /* Catégories */
  .comp-sections { display: flex; flex-direction: column; gap: 16px; }
  .cat-section { background: rgba(255,255,255,.02); border: 1px solid rgba(99,102,241,.14); border-radius: 16px; overflow: hidden; }
  .cat-section.cat-done { border-color: rgba(16,185,129,.25); }
  .cat-hd { display: flex; align-items: center; gap: 10px; padding: 14px 16px; background: rgba(99,102,241,.06); border-bottom: 1px solid rgba(99,102,241,.1); }
  .cat-done .cat-hd { background: rgba(16,185,129,.06); border-bottom-color: rgba(16,185,129,.1); }
  .cat-ico { font-size: 18px; line-height: 1; }
  .cat-nm { font: 700 14px/1 'Plus Jakarta Sans', sans-serif; color: #fff; flex: 1; }
  .cat-cnt { font: 700 12px/1 'IBM Plex Mono', monospace; color: #94a3b8; }

  /* Compétences */
  .comp-list { display: flex; flex-direction: column; }
  .comp-row { display: flex; align-items: center; gap: 10px; padding: 13px 16px; cursor: pointer; border-bottom: 1px solid rgba(255,255,255,.04); transition: background .15s; }
  .comp-row:last-child { border-bottom: none; }
  .comp-row:not(.comp-done):hover { background: rgba(99,102,241,.08); }
  .comp-row:active { transform: scale(.99); }
  .comp-row.comp-done { opacity: .5; cursor: default; }
  .comp-row.comp-sel { background: rgba(99,102,241,.15); }
  .comp-code { font: 700 11px/1 'IBM Plex Mono', monospace; color: #6366f1; background: rgba(99,102,241,.15); border-radius: 6px; padding: 4px 7px; flex-shrink: 0; }
  .comp-nom { font: 500 13px/1.4 'Inter', sans-serif; color: #e2e8f0; flex: 1; }
  .comp-status { flex-shrink: 0; }
  .badge-ok { font: 600 11px/1 'IBM Plex Mono', monospace; color: #10b981; background: rgba(16,185,129,.12); border-radius: 20px; padding: 3px 8px; }
  .badge-sel { font: 600 11px/1 'IBM Plex Mono', monospace; color: #6366f1; background: rgba(99,102,241,.2); border-radius: 20px; padding: 3px 8px; }

  /* CTA sticky */
  .cta-slot { position: fixed; bottom: 0; left: 0; right: 0; z-index: 100; padding: 12px 16px; padding-bottom: max(12px, env(safe-area-inset-bottom)); background: rgba(10,13,26,.9); backdrop-filter: blur(16px); border-top: 1px solid rgba(99,102,241,.2); animation: ctaUp .25s ease; }
  @keyframes ctaUp { from { transform: translateY(100%); opacity: 0; } to { transform: translateY(0); opacity: 1; } }
  .cta-strip { display: flex; align-items: center; gap: 14px; max-width: 580px; margin: 0 auto; }
  .cta-info { flex: 1; min-width: 0; }
  .cta-comp-nm { font: 700 14px/1.3 'Plus Jakarta Sans', sans-serif; color: #fff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .cta-for { font: 500 12px/1 'Inter', sans-serif; color: #94a3b8; margin-top: 3px; }
  .btn-validate { padding: 14px 22px; background: linear-gradient(135deg, #6366f1, #8b5cf6); border: 0; border-radius: 14px; color: #fff; font: 800 14px/1 'Plus Jakarta Sans', sans-serif; cursor: pointer; white-space: nowrap; box-shadow: 0 6px 20px rgba(99,102,241,.4); transition: all .2s; flex-shrink: 0; }
  .btn-validate:disabled { opacity: .5; cursor: not-allowed; box-shadow: none; }
  .btn-validate:not(:disabled):active { transform: scale(.97); }

  .empty { padding: 40px 20px; text-align: center; color: #64748b; font: 500 14px/1.5 'Inter', sans-serif; }
...
async function doValidate() {
  ...
  toast('✅ Validée ! Élève notifié pour le quiz.', 'success');
...
function renderCta() {
  ...
  slot.innerHTML = `
    <div class="cta-strip">
      <div class="cta-info">
        <div class="cta-comp-nm">${esc(_selectedComp.n)}</div>
